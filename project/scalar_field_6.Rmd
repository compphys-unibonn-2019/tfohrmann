...
```{r}
corr <- array(data=NA, dim=c(10,n_t))
for(i in seq(10,100,10)){
  ...
  corr[i/10,] <- two_point(lattice, mass, field, zeta)
}
...
```{r}
#print(mean_corr)
plot(seq(1,length(corr[10,])),corr[10,])
...
for(i in seq(1,150,1)){
  tmp_vec <- scan(file=paste(c("./m1f0/sweep_", i-1, ".txt"), collapse = ""), sep=",", quiet=T)
  tmp_arr <- array(tmp_vec[-length(tmp_vec)], dim=c(32,16,16,16))
